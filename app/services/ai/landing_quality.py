...
def score_landing_page(landing: dict[str, Any], *, mobile_heavy: bool = True) -> dict[str, Any]:
    ...
    present = _present(landing)
    total_w = sum(w for _, _, w in _CHECKS)
    got = sum(w for key, _, w in _CHECKS if present[key])
    score = round(got / total_w * 100) if total_w else 0
    grade = "A" if score >= 85 else "B" if score >= 70 else "C" if score >= 50 else "D"

    checks = [
        {"item": label, "ok": present[key], "weight": w}
        for key, label, w in _CHECKS
    ]
    # Suggestions only for what's missing, heaviest lever first.
    missing = sorted(
        [(key, w) for key, _, w in _CHECKS if not present[key]],
        key=lambda kw: kw[1],
        reverse=True,
    )
    suggestions = [_FIX[key] for key, _ in missing]
    if mobile_heavy:
